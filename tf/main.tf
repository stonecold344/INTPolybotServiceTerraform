...
resource "aws_instance" "polybot_instance" {
  ...
  user_data = <<-EOF
                ...
                # Login to DockerHub
                sudo docker login -u "DOCKERHUB_USERNAME" -p "$SECRET_VALUE"

                # Pull the latest Polybot image
                sudo docker pull DOCKERHUB_USERNAME/polybot:latest

                # Run Polybot in Docker
                sudo docker run -d --name polybot -p 8443:8443 --restart always DOCKERHUB_USERNAME/polybot:latest
                EOF
...
resource "aws_instance" "yolo5_instance" {
  ...
  user_data = <<-EOF
                ...
                sudo docker pull DOCKERHUB_USERNAME/yolo5:latest

                # Run Yolo5 in Docker
                sudo docker run -d --name yolo5 -p 8081:8081 --restart always DOCKERHUB_USERNAME/yolo5:latest
                EOF
